    ...
    if delta > 0 then
        for i = 1, #self:GetParent().columns do
            if IsShiftKeyDown() then
                self:GetParent().columns[i]:ScrollToTop()
            else
                self:GetParent().columns[i]:ScrollUp()
            end
        end
    elseif delta < 0 then
        for i = 1, #self:GetParent().columns do
            if IsShiftKeyDown() then
                self:GetParent().columns[i]:ScrollToBottom()
            else
                self:GetParent().columns[i]:ScrollDown()
            end
        end
    end
end
...
for i = 1, 3 do
    ...
    if i == 1 or i == 3 then
        smf:SetWidth(LanCombat.frame:GetWidth()/3 - 45)
    else
        smf:SetWidth(LanCombat.frame:GetWidth()/3 + 82)
    end
    LanCombat.frame.columns[i] = smf
end

LanCombat.frame.columns[1]:SetPoint('TOPLEFT')
LanCombat.frame.columns[1]:SetPoint('BOTTOMLEFT')
LanCombat.frame.columns[2]:SetPoint('TOP')
LanCombat.frame.columns[2]:SetPoint('BOTTOM')
LanCombat.frame.columns[3]:SetPoint('TOPRIGHT')
LanCombat.frame.columns[3]:SetPoint('BOTTOMRIGHT')

LanCombat.frame.columns[1]:SetJustifyH('LEFT')
...
local Output = function(frame, rsaFrame, color, text, rsaText, critical, pet, prefix, suffix, tooltipMsg, throttle, noccl)
    ...
    if LanScroll and (not(throttle) or (throttle and noccl)) then
        ...
        LanScroll:AddText(rsamsg, critical, rsaFrame and rsaFrame or frame == 1 and 'Incoming' or frame == 3 and 'Outgoing' or 'Information')
    end
end
