--~ DataFeed = CreateFrame('frame')
--~ DataFeed:SetPoint('TOPLEFT', MultiBarBottomLeftButton1, 'BOTTOMLEFT', 0, -25)
--~ DataFeed:SetPoint('TOPRIGHT', MultiBarBottomLeftButton12, 'BOTTOMRIGHT', 3, -25)
--~ DataFeed:SetHeight(30)
...
local function GuildTip(self)
    ...
    local col = LanFunc.RGBToHex(1, 0, 1)
	...
	if (GetGuildLevel() ~= 25) then
		...
		GameTooltip:AddLine(format(col..GUILD_EXPERIENCE_CURRENT, '|r |cFFFFFFFF'..LanFunc.ShortValue(currentXP), LanFunc.ShortValue(nextLevelXP), percentTotal))
		GameTooltip:AddLine(format(col..GUILD_EXPERIENCE_DAILY, '|r |cFFFFFFFF'..LanFunc.ShortValue(dailyXP), LanFunc.ShortValue(maxDailyXP), percentDaily))
	end
	...
	if (standingID ~= 4) then
		...
		GameTooltip:AddLine(format('|cFFFFFFFF%s/%s (%s%%)', LanFunc.ShortValue(curr), LanFunc.ShortValue(max), ceil((curr / max) * 100)))
	end
		...
		for i = 1, #guildTable do
			...
			if (connected and name ~= select(1, UnitName('player'))) then
                ...
                if (IsShiftKeyDown()) then
                    local pii = LanFunc.RGBToHex(1, 0, 1)
					...
					if (officernote ~= '') then 
                        local oCOL = LanFunc.RGBToHex(0.3, 1, 0.15)
